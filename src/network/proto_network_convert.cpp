		...
		namespace network
		{
			...
			shared_ptr <News> ProtoNetworkConvert::ToProto(const uint8_t* buffer, const size_t)
			{
				...
				if (network_data.header.content_compress_type)
				{
					...
					string str(length, '\0');
					str.resize(qlz_decompress(network_data.content.c_str(), const_cast <char *>(str.data()), make_shared <qlz_state_decompress>().get()));
			...
			bool ProtoNetworkConvert::ToNetwork(const shared_ptr <Message> message, string& out_buffer, const int session_id)
			{
				...
				auto content = message->SerializeAsString();
				if (content.length() > kMaxUncompressSize)
				{
					network_data.content.resize(content.length());
					network_data.content.resize(qlz_compress(content.c_str(), const_cast <char *>(network_data.content.data())
						, content.length(), make_shared <qlz_state_compress>().get()));
					network_data.header.content_compress_type = 1;
				}
				else
				{
					network_data.header.content_compress_type = 0;
					network_data.content.swap(content);
				}
